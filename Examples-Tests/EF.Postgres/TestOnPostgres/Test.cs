...
public class Test(DatabaseAndAppFixture appFixture) : IClassFixture<DatabaseAndAppFixture>
{
    ...
    public Task<(WeakReference @ref, int affected)> InsertManyRecordsInScopedContextOnCurrentTranAndMarkLastObjectInRAMAsync(IServiceScopeFactory serviceScopeFactory, string value, int count) =>
        appFixture.DB.Database.ConstructScopedContextOnCurrentTranAsync<PostgresContext, (WeakReference, int)>(serviceScopeFactory, async (db, scope) =>
        {
    ...
    public async Task TestPostgres()
    {
        ...
        //Act
        await using var tran = await appFixture.DB.Database.BeginTransactionAsync(System.Data.IsolationLevel.ReadCommitted);
        ...
        var checkContext = checkScope.ServiceProvider.GetRequiredService<PostgresContext>();
        await using var checkTran = await checkContext.Database.BeginTransactionAsync(System.Data.IsolationLevel.ReadCommitted);
